# TODO: Migrate Termix PVC from local-path to NAS-backed storage
...
### 1. Add the new PV + PVC alongside the old one (declarative)
...
---
# Old local-path PVC — kept during migration, removed in step 5.
apiVersion: v1
...
Note: `nfs.path: /` serves the whole NAS root (same as copyparty); subdir isolation happens pod-side via `subPath` in step 4. If you want on-PV isolation instead, set `nfs.path: /termix` and drop the `subPath` later — but that only works if rclone exports the subdir correctly.
...
Termix is offline from this point until step 4 completes.

### 3. Copy data with a helper pod
...
### 4. Point Termix at the new PVC + scale up
...
### 5. Visually verify in Termix, then clean up
...
At any point before step 5 (clean-up), recovery is trivial: edit `apps/termix/deployment.yaml` to point `claimName` back at `termix-data`, reconcile, scale up. The old PVC was never touched, still bound to its local PV, still has the original data. The NAS PVC can stay around as an empty-but-Bound resource or be deleted.

After step 5, the old PV is gone — rollback requires restoring from whatever backup you have. Which is why step 5 only happens after visual verification in the UI.
